    ...
    with torch.no_grad(), precision_scope(opt.device), model.ema_scope():
        ...
        for n in trange(opt.n_iter, desc='Sampling'):
            for prompts in tqdm(data, desc='data'):
                ...
                c = model.get_learned_conditioning(prompts)
                shape = [opt.C, opt.H // opt.f, opt.W // opt.f]
                samples, _ = sampler.sample(S=opt.steps, conditioning=c, batch_size=opt.n_samples, shape=shape, verbose=False,
                                            unconditional_guidance_scale=opt.scale, unconditional_conditioning=uc, eta=opt.ddim_eta,
                                            x_T=start_code)
